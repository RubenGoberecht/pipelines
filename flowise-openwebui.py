...
class Pipeline:  # Changed from Pipe to Pipeline
    ...
    def __init__(self):
        ...
        self.tools = None  # Added to match the working example
        self.valves = self.Valves()
        self.pipelines = self.get_available_pipes()  # Changed to set self.pipelines
    ...
    def get_available_pipes(self) -> List[dict]:  # Renamed from pipes()
    ...
    def pipe(self, user_message: str, model_id: str, messages: List[dict], body: dict):  # Updated signature
        """Process chat messages through Flowise"""
        try:
            print("\nProcessing Flowise request:")
            print(f"Request body: {json.dumps(body, indent=2)}")
            
            # Get the current message (from user_message parameter)
            question = user_message
            
            # Process previous messages as history
            history = []
            if messages:  # If we have previous messages
                for msg in messages:
